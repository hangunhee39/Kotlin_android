...
import hgh.project.movie_grade.databinding.ItemMovieInformationBinding
import hgh.project.movie_grade.databinding.ItemMyReviewBinding
import hgh.project.movie_grade.databinding.ItemReviewBinding
import hgh.project.movie_grade.databinding.ItemReviewFormBinding
import hgh.project.movie_grade.domain.model.Movie
import hgh.project.movie_grade.domain.model.Review
import hgh.project.movie_grade.extension.toAbbreviatedString
import hgh.project.movie_grade.extension.toDecimalFormatString

//영와정보 따로 리뷰 따로 구현하는 방법
class MovieReviewsAdapter(private val movie: Movie) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun getItemCount(): Int = 2 + reviews.size //header+(내 리뷰 상태) 때문에 더함
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            ...
            is ReviewViewHolder -> {
                holder.bind(reviews[position - 2])  //header+(내 리뷰 상태) 빼기
            }
